...
func TestToSqlite3AlterTable(t *testing.T) {
	{
		statement := `
			ALTER TABLE
				...
				ADD COLUMN sql_delay INT UNSIGNED NOT NULL AFTER replication_downstream_lag
		`
	...
	}
	{
		...
		test.S(t).ExpectEquals(result, stripSpaces(`
			create index
				master_host_port_idx_ham_database_instance
				on ham_database_instance (master_host, master_port)
			`))
	}
	{
		statement := `
...
func TestCreateIndex(t *testing.T) {
	{
		statement := `
			create index
				...
				on ham_database_instance (master_host(128), master_port)
		`
		result := stripSpaces(ToSqlite3Dialect(statement))
		test.S(t).ExpectEquals(result, stripSpaces(`
			create index
