...
func Setup(applier CommandApplier, thisHostname string) error {
	...
	store = NewStore(config.Config.RaftDataDir, raftBind, raftAdvertise, applier, dtstruct.SnapshotHandlerMap[constant.SnapshotTypeRDMS])
	peerNodes := []string{}
...
func Monitor() {
	...
	for {
		...
		case <-followerHealthTick:
			if IsLeader() {
				athenticationToken := dtstruct.NewToken().Short()
