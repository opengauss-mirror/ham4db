...
var createQueries = []string{
	...
	`,
	`
		CREATE INDEX IF NOT EXISTS store_key_idx_raft_store ON ham_raft_store (store_key)
	`,
}
...
func (relStore *RelationalStore) openDB() (*sql.DB, error) {
	...
	if relStore.backend == nil {
		...
		sqliteDB, _, err := sqlutil.GetSQLiteDB(relStoreFile)
		if err != nil {
			return nil, err
		}
		...
		for _, query := range createQueries {
			if _, err := sqliteDB.Exec(sqlutil.ToSqlite3Dialect(query)); err != nil {
				return nil, err
			}
		}
...
func (relStore *RelationalStore) Set(key []byte, val []byte) error {
	...
	stmt, err := tx.Prepare(`delete from ham_raft_store where store_key = ?`)
	if err != nil {
		return err
	}
	_, err = stmt.Exec(key)
	...
	stmt, err = tx.Prepare(`insert into ham_raft_store (store_key, store_value) values (?, ?)`)
	if err != nil {
		tx.Rollback()
		return err
	}
...
func (relStore *RelationalStore) Get(key []byte) (val []byte, err error) {
	...
	err = db.QueryRow("select min(store_value) from ham_raft_store where store_key = ?", key).Scan(&val)
	return val, err
}
...
func (relStore *RelationalStore) FirstIndex() (idx uint64, err error) {
	...
	err = db.QueryRow("select ifnull(min(log_index), 0) from ham_raft_log").Scan(&idx)
	return idx, err
}
...
func (relStore *RelationalStore) LastIndex() (idx uint64, err error) {
	...
	err = db.QueryRow("select ifnull(max(log_index), 0) from ham_raft_log").Scan(&idx)
	return idx, err
}
...
func (relStore *RelationalStore) DeleteRange(min, max uint64) error {
	...
	_, err = db.Exec("delete from ham_raft_log where log_index >= ? and log_index <= ?", min, max)
	return err
}
