...
func WriteRegisterNode(nodeHealth *NodeHealth) (healthy bool, err error) {
	...
	nodeHealth.onceHistory.Do(func() {
		db.ExecSQL(`
			insert ignore into ham_node_health_history
				(hostname, token, first_seen_active_timestamp, extra_info, command, app_version)
			values
				(?, ?, NOW(), ?, ?, ?)
			`,
	...
	// Got here? The UPDATE didn't work. Row isn't there.
	{
		...
		if config.Config.IsSQLite() {
			...
			dbBackend = fmt.Sprintf("%s:%d", config.Config.BackendDBHost,
				config.Config.BackendDBPort)
		}
		sqlResult, err := db.ExecSQL(`
			insert ignore into ham_node_health
				(hostname, token, first_seen_active_timestamp, last_seen_active_timestamp, extra_info, command, app_version, db_backend)
			values (
				?, ?,
...
// the ham4db active node.
func ExpireNodesHistory() error {
	_, err := db.ExecSQL(`
			delete
				from ham_node_health_history
			where
				first_seen_active_timestamp < now() - interval ? hour
			`,
...
func ReadAvailableNodes(onlyHttpNodes bool) (nodes [](*NodeHealth), err error) {
	...
	query := `
		select
			hostname, token, app_version, first_seen_active_timestamp, last_seen_active_timestamp, db_backend
		from
			ham_node_health
		where
			last_seen_active_timestamp > now() - interval ? second
	...
	err = db.Query(query, sqlutil.Args(config.HealthPollSeconds*2, extraInfo), func(m sqlutil.RowMap) error {
		nodeHealth := &NodeHealth{
			Hostname:        m.GetString("hostname"),
			Token:           m.GetString("token"),
			AppVersion:      m.GetString("app_version"),
			FirstSeenActive: m.GetString("first_seen_active_timestamp"),
			LastSeenActive:  m.GetString("last_seen_active_timestamp"),
...
func TokenBelongsToHealthyHttpService(token string) (result bool, err error) {
	...
	err = db.Query(query, sqlutil.Args(token, extraInfo), func(m sqlutil.RowMap) error {
