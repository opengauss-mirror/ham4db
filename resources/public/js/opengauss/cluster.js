...
function Cluster() {
    ...
    var _instanceCommands = {
        "recover-auto": function (e) {
            apiGetCommand(true, "/api/recover/" + _instancesMap[e.draggedNodeId].Key.Hostname + "/" + _instancesMap[e.draggedNodeId].Key.Port);
            return true;
        },
        "recover-auto-lite": function (e) {
            apiGetCommand(true, "/api/recover-lite/" + _instancesMap[e.draggedNodeId].Key.Hostname + "/" + _instancesMap[e.draggedNodeId].Key.Port);
            return true;
        },
        "force-master-failover": function (e) {
            apiGetCommand(true, "/api/force-master-failover/" + _instancesMap[e.draggedNodeId].Key.Hostname + "/" + _instancesMap[e.draggedNodeId].Key.Port);
            return true;
        },
        "recover-suggested-successor": function (e) {
            ...
            apiGetCommand(true, "/api/recover/" + _instancesMap[e.draggedNodeId].Key.Hostname + "/" + _instancesMap[e.draggedNodeId].Key.Port + "/" + suggestedSuccessorHost + "/" + suggestedSuccessorPort);
            return true;
        },
        "relocate-replicas": function (e) {
            ...
            apiGetCommand(true, "/api/relocate-replicas/" + _instancesMap[e.draggedNodeId].Key.Hostname + "/" + _instancesMap[e.draggedNodeId].Key.Port + "/" + belowHost + "/" + belowPort);
            return true;
        },
    ...
    function generateInstanceDiv(svgInstanceWrapper, nodesMap) {
        ...
        opengaussRenderInstanceElement(instanceEl, node, "cluster");
    ...
    function prepareInstanceMasterSectionDroppable(instanceMasterSectionEl) {
        ...
        instanceMasterSectionEl.droppable({
            accept: function (draggable) {
                ...
                var action = _isDraggingTrailer ? moveDownstream : moveUpstream;
                var acceptDrop = action(draggedNode, targetNode, false);
                var instanceDiv = $(this).closest(".instance");
                instanceDiv.addClass("accept_drop_check");
                if (acceptDrop.accept == "ok") {
                ...
                $(this).attr("data-drop-comment", acceptDrop.type);
                var accepted = acceptDrop.accept != null;
                return accepted;
            },
            hoverClass: "draggable-hovers",
            ...
            out: function (event, ui) {
                ...
                // Called once when dragged object leaves other object
                $(duplicate).removeClass("draggable-msg");
                $(duplicate).find(".instance-content,.instance-trailer-content").html("Can not drop here")
            },
            drop: function (e, ui) {
                var draggedNodeId = ui.draggable.attr("data-nodeid");
                var duplicate = ui.helper;
    ...
    //             };
    //         }
    //         if (droppableNode.UpstreamKey.Hostname && droppableNode.UpstreamKey.Hostname != "_") {
    ...
    function moveUpstream(node, droppableNode, shouldApply) {
        var unaccepted = {
            accept: false,
        };
        if (!isAuthorizedForAction()) {
        ...
        var droppableTitle = getInstanceDiv(droppableNode.id).find("h3 .pull-left").html();
        var typeUndo = '<span class="text-warning"><strong>nothing to do from ' + node.Role + ' to ' + droppableNode.Role + '</strong></span><br>' + droppableTitle
        var typeDo = '<span class="text-success"><strong>switch over from ' + node.Role + ' to ' + droppableNode.Role + '</strong></span><br>' + droppableTitle

        // if (node.hasConnectivityProblem || droppableNode.hasConnectivityProblem || droppableNode.isAggregate) {
        //     return {
        //         accept: false,
        //         type: '<span class="text-warning"><strong>node has connection problem, operation is not allowed</strong></span>'
        //     };
        // }

        // role -> role
        // same role do not need to change topology
        if (node.Role === droppableNode.Role) {
            return {
                accept: false,
                type: typeUndo
            };
        ...
        if (instanceIsChild(node, droppableNode) && !droppableNode.isMaster && !droppableNode.isCoMaster) {
            if (node.hasProblem || node.hasConnectivityProblem) {
                return {
                    accept: false,
                    type: '<span class="text-warning"><strong>node has problem, operation is not allowed</strong></span>'
                };
            }

            if (shouldApply) {
                SwitchOver(node, droppableNode);
            }
            return {
                accept: "ok",
                type: typeDo
        ...
        // standby -> primary
        if (instanceIsChild(node, droppableNode) && droppableNode.isMaster && !node.isCoMaster) {
            if (node.hasProblem || node.hasConnectivityProblem) {
                return {
                    accept: false,
                    type: '<span class="text-warning"><strong>node has problem, operation is not allowed</strong></span>'
    ...
    // When shouldApply is false nothing gets executed, and the function merely serves as a predictive
    // to the possibility of the drop.
    function moveDownstream(node, droppableNode, shouldApply) {
        if (!isAuthorizedForAction()) {
            // Obviously this is also checked on server side, no need to try stupid hacks
            return {
                accept: false
    ...
    function executeOperation(message, apiUrl, isTypePre, type) {
        if (isSilentUI()) {
            apiCommand(isTypePre, type, apiUrl);
        } else {
    ...
    function executeMoveOperation(message, apiUrl, isTypePre) {
        if (isSilentUI()) {
            apiGetCommand(isTypePre, apiUrl);
        } else {
            bootbox.confirm(anonymizeIfNeedBe(message), function (confirm) {
                if (confirm) {
                    apiGetCommand(isTypePre, apiUrl);
    ...
    function relocate(node, siblingNode) {
        var message = "<h4>Relocate</h4>Are you sure you wish to turn <code><strong>" +
            ...
            "</strong></code>?" +
            "<h4>Note</h4><p>Ham4db will try and figure out the best relocation path. This may involve multiple steps. " +
    ...
    function relocateReplicas(node, siblingNode, pattern) {
        ...
        var message = "<h4>relocate-replicas</h4>Are you sure you wish to relocate replicas of <code><strong>" +
            ...
            siblingNode.Key.Hostname + ":" + siblingNode.Key.Port +
            "</strong></code>?" +
            "<h4>Note</h4><p>Will try and figure out the best relocation path. This may involve multiple steps. " +
    ...
    function SwitchOver(node, droppableNode) {
        var message = "<h4>Switch Over</h4>Are you sure to promote <code><strong>" +
            node.Key.Hostname + ":" + node.Key.Port +
            "</strong></code> as " + droppableNode.Role + "?";
        var apiUrl = "/api/opengauss/switchover/" + node.ClusterId + "/" + node.Key.Hostname + "/" + node.Key.Port;
        return executeOperation(message, apiUrl, false, "put");
    }
    ...
    function makeCoMaster(node, childNode) {
        ...
        bootbox.confirm(anonymizeIfNeedBe(message), function (confirm) {
            if (confirm) {
                apiGetCommand(true, "/api/make-co-master/" + childNode.Key.Hostname + "/" + childNode.Key.Port);
                return true;
            }
        });
        return false;
    ...
    function gracefulMasterTakeover(newMasterNode, existingMasterNode) {
        var message = '<h4><span class="glyphicon glyphicon-exclamation-sign text-warning"></span> DANGER ZONE</h4>Are you sure to promote <code><strong>' +
            newMasterNode.Key.Hostname + ':' + newMasterNode.Key.Port +
            '</strong></code> as primary gracefully?';
        bootbox.confirm(anonymizeIfNeedBe(message), function (confirm) {
            if (confirm) {
                apiGetCommand(true, "/api/graceful-master-takeover/" + existingMasterNode.Key.Hostname + "/" + existingMasterNode.Key.Port + "/" + newMasterNode.Key.Hostname + "/" + newMasterNode.Key.Port);
                return true;
            }
        });
        return false;
    ...
    function instanceIsChild(node, parentNode) {
        return node.UpstreamKey.Hostname === parentNode.Key.Hostname;
    }
    ...
    function promptForAlias(oldAlias) {
        bootbox.prompt({
            ...
            callback: function (result) {
                if (result !== null) {
                    showLoader();
                    getData(false, "/api/set-cluster-alias/" + currentClusterName() + "?alias=" + encodeURIComponent(result), function (operationResult) {
                        hideLoader();
    ...
    function showOSCReplicas() {
        getData(false, "/api/cluster-osc-replicas/" + currentClusterName(), function (instances) {
    ...
    function populateSidebar(clusterInfo) {
        ...
        var maxItems = 5
        getData(true, "/api/audit-recovery/alias/" + clusterInfo.ClusterAlias, function (recoveries) {
            recoveries = recoveries || []
            recoveries = recoveries.slice(0, maxItems)
            if (recoveries.length > 0) {
                var content = '<a href="' + appUrl(true, '/web/audit-recovery/alias/' + clusterInfo.ClusterAlias) + '">Recovery history</a>';
        ...
        getData(true, "/api/audit-failure-detection/alias/" + clusterInfo.ClusterAlias, function (failureDetections) {
            failureDetections = failureDetections || []
            failureDetections = failureDetections.slice(0, maxItems)
            if (failureDetections.length > 0) {
                var content = '<a href="' + appUrl(true, '/web/audit-failure-detection/alias/' + clusterInfo.ClusterAlias) + '">Failure detection</a>';
    ...
    function onAnalysisEntry(analysisEntry, instance) {
        ...
        if (!instance.isMaster) {
            ...
            recoveryListing.append('<li role="separator" class="divider"></li>');
            recoveryListing.append('<li><a href="#" data-btn="relocate-replicas" data-command="relocate-replicas" data-successor-host="' + instance.UpstreamKey.Hostname + '" data-successor-port="' + instance.UpstreamKey.Port + '">Relocate replicas to <code>' + instance.masterTitle + '</code></a></li>');
        }
    ...
    function reviewReplicationAnalysis(replicationAnalysis) {
        ...
        if (replicationAnalysis.Detail != null) {
            replicationAnalysis.Detail.forEach(function (analysisEntry) {
    ...
    function indicateClusterPoolInstances(clusterPoolInstances) {
        var instancesMap = _instancesMap;
        for (var pool in clusterPoolInstances.Detail) {
            if (clusterPoolInstances.Detail.hasOwnProperty(pool)) {
                clusterPoolInstances.Detail[pool].forEach(function (instanceKey) {
    ...
    function renderCluster() {
        ...
        _instancesMap = opengaussNormalizeInstances(instances, maintenanceList);
        if (isCompactDisplay()) {
        ...
        instances.forEach(function (instance) {
            if (instance.isMaster) {
                getData(true, "/api/recently-active-instance-recovery/" + instance.Key.Hostname + "/" + instance.Key.Port, function (recoveries) {
                    if (!recoveries) {
                        return
                    }
        ...
        if ($.cookie("pool-indicator") == "true") {
            getData(true, "/api/cluster-pool-instances/" + currentClusterName(), function (clusterPoolInstances) {
    ...
    function domReady() {
        getData(true, "/api/cluster/" + currentClusterName(), function (instances) {
            _instances = instances;
            getData(true, "/api/replication-analysis/" + currentClusterName(), function (replicationAnalysis) {
                _replicationAnalysis = replicationAnalysis;
                getData(true, "/api/maintenance", function (maintenanceList) {
                    _maintenanceList = maintenanceList;
                    $(document).trigger('ham4db:preRenderCluster');
                    renderCluster();
                    $(document).trigger('ham4db:postRenderCluster');
                });
            });
        });
        getData(true, "/api/cluster-info/" + currentClusterName(), function (clusterInfo) {
            ...
            $("#dropdown-context").append('<li><a href="' + appUrl(true, '/web/cluster-pools/' + currentClusterName()) + '">Pools</a></li>');
            if (isCompactDisplay()) {
        ...
        getData(true, "/api/active-cluster-recovery/" + currentClusterName(), function (recoveries) {
            // Result is an array: either empty (no active recovery) or with multiple entries
            if (!recoveries) {
                return
            }
            recoveries.forEach(function (recoveryEntry) {
                addInfo('<strong><a href="' + appUrl(true, '/web/audit-recovery/cluster/' + currentClusterName()) + '">' + recoveryEntry.AnalysisEntry.Analysis + ' active recovery in progress</strong></a>. Topology is subject to change in the next moments.');
            });
        });
        getData(true, "/api/recently-active-cluster-recovery/" + currentClusterName(), function (recoveries) {
            if (!recoveries) {
                return
            }
            // Result is an array: either empty (no active recovery) or with multiple entries
            var recoveryEntry = recoveries[0]
            // TODO
            // addInfo('This cluster just recently (' + recoveryEntry.RecoveryEndTimestamp + ') recovered from <strong><a href="' + appUrl(true, '/web/audit-recovery/cluster/' + currentClusterName()) + '">' + recoveryEntry.AnalysisEntry.Analysis + '</strong></a>. It may still take some time to rebuild topology graph.');
        });
        getData(true, "/api/blocked-recoveries/cluster/" + currentClusterName(), function (blockedRecoveries) {
            // Result is an array: either empty (no active recovery) or with multiple entries
            blockedRecoveries.forEach(function (blockedRecovery) {
                addAlert('A <strong>' + blockedRecovery.Analysis + '</strong> on ' + getInstanceTitle(blockedRecovery.FailedInstanceKey.Hostname, blockedRecovery.FailedInstanceKey.Port) + ' is blocked due to a <a href="' + appUrl(true, '/web/audit-recovery/id/' + blockedRecovery.BlockingRecoveryId) + '">previous recovery</a>');
            });
        });
    ...
    function getData(isTypePre, url, cb) {
        $.get(appUrl(isTypePre, url), cb, "json");
    }
