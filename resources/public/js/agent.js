...
$(document).ready(function() {
  ...
  $.get(appUrl(false,"/api/agent/" + currentAgentHost()), function(agent) {
    showLoader();
  ...
  $.get(appUrl(false,"/api/agent-active-seeds/" + currentAgentHost()), function(activeSeeds) {
    showLoader();
    ...
    if (activeSeeds.length > 0) {
      ...
      $.get(appUrl(false,"/api/agent-seed-states/" + activeSeeds[0].SeedId), function(seedStates) {
        showLoader();
  ...
  $.get(appUrl(false,"/api/agent-recent-seeds/" + currentAgentHost()), function(recentSeeds) {
    showLoader();
  ...
  function displayAgent(agent) {
    ...
    $("[data-agent=hostname_search]").html(
      '<a href="' + appUrl(true,'/web/search?s=' + agent.Hostname + ':' + agent.MySQLPort) + '">' + agent.Hostname + '</a>' + '<div class="pull-right"><button class="btn btn-xs btn-success" data-command="discover" data-hostname="' + agent.Hostname + '" data-mysql-port="' + agent.MySQLPort + '">Discover</button></div>'
    );
    ...
    function beautifyAvailableSnapshots(hostnames) {
      ...
      result = result.map(function(hostname) {
        ...
        return '<td><a href="' + appUrl(false,'/web/agent/' + hostname) + '">' + hostname + '</a><div class="pull-right"><button class="btn btn-xs ' + btnType + '" data-command="seed" data-seed-source-host="' + hostname + '" data-seed-local="' + isLocal + '" data-mysql-running="' + agent.MySQLRunning + '">Seed</button></div></td>';
      });
  ...
  $("body").on("click", "button[data-command=unmount]", function(event) {
    ...
    showLoader();
    $.get(appUrl(false,"/api/agent-umount/" + currentAgentHost()), function(operationResult) {
      hideLoader();
  ...
  $("body").on("click", "button[data-command=mountlv]", function(event) {
    ...
    showLoader();
    $.get(appUrl(false,"/api/agent-mount/" + currentAgentHost() + "?lv=" + encodeURIComponent(lv)), function(operationResult) {
      hideLoader();
      if (operationResult.Code == "ERROR") {
        addAlert(operationResult.Message)
      } else {
  ...
  $("body").on("click", "button[data-command=removelv]", function(event) {
    ...
    bootbox.confirm(message, function(confirm) {
      if (confirm) {
        showLoader();
        $.get(appUrl(false,"/api/agent-removelv/" + currentAgentHost() + "?lv=" + encodeURIComponent(lv)), function(operationResult) {
          hideLoader();
          if (operationResult.Code == "ERROR") {
            addAlert(operationResult.Message)
          } else {
  ...
  $("body").on("click", "button[data-command=create-snapshot]", function(event) {
    ...
    bootbox.confirm(message, function(confirm) {
      if (confirm) {
        showLoader();
        $.get(appUrl(false,"/api/agent-create-snapshot/" + currentAgentHost()), function(operationResult) {
          hideLoader();
          if (operationResult.Code == "ERROR") {
            addAlert(operationResult.Message)
          } else {
  ...
  $("body").on("click", "button[data-command=mysql-stop]", function(event) {
    ...
    bootbox.confirm(message, function(confirm) {
      if (confirm) {
        showLoader();
        $.get(appUrl(false,"/api/agent-mysql-stop/" + currentAgentHost()), function(operationResult) {
          hideLoader();
          if (operationResult.Code == "ERROR") {
            addAlert(operationResult.Message)
          } else {
  ...
  $("body").on("click", "button[data-command=mysql-start]", function(event) {
    showLoader();
    $.get(appUrl(false,"/api/agent-mysql-start/" + currentAgentHost()), function(operationResult) {
      hideLoader();
      if (operationResult.Code == "ERROR") {
        addAlert(operationResult.Message)
      } else {
  ...
  $("body").on("click", "button[data-command=seed]", function(event) {
    ...
    bootbox.confirm(message, function(confirm) {
      if (confirm) {
        showLoader();
        $.get(appUrl(false,"/api/agent-seed/" + currentAgentHost() + "/" + sourceHost), function(operationResult) {
          hideLoader();
          if (operationResult.Code == "ERROR") {
            addAlert(operationResult.Message)
          } else {
