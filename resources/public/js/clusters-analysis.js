...
$(document).ready(function() {
  ...
  $.get(appUrl(true,"/api/clusters-info"), function(clusters) {
    clusters = clusters || [];
    $.get(appUrl(true,"/api/replication-analysis"), function(replicationAnalysis) {
      $.get(appUrl(true,"/api/blocked-recoveries"), function(blockedRecoveries) {
  ...
  function displayClustersAnalysis(clusters, replicationAnalysis, blockedRecoveries) {
    ...
    function displayCluster(cluster) {
      $("#clusters_analysis").append('<div xmlns="http://www.w3.org/1999/xhtml" class="popover instance right" data-cluster-name="' + cluster.ClusterName + '"><div class="arrow"></div><h3 class="popover-title"><div class="pull-left"><a href="' + appUrl(true,'/web/cluster/' + cluster.ClusterName) + '"><span>' + cluster.ClusterName + '</span></a></div><div class="pull-right"></div>&nbsp;<br/>&nbsp;</h3><div class="popover-content"><div></div></div></div>');
      ...
      if (cluster.ClusterAlias != "") {
        ...
        popoverElement.find("h3 .pull-left").prepend('<a href="' + appUrl(true,'/web/cluster/alias/' + encodeURIComponent(cluster.ClusterAlias)) + '"><strong>' + cluster.ClusterAlias + '</strong></a><br/>');
        popoverElement.find("h3 .pull-right").append('<a href="' + appUrl(true,'/web/cluster/alias/' + encodeURIComponent(cluster.ClusterAlias) + '?compact=true') + '"><span class="glyphicon glyphicon-compressed" title="Compact display"></span></a>');
      }
    ...
    if (clusters.length == 0) {
      // No problems
      var info = "No incidents which require a failover to report. Reports the following incidents:<ul>";
