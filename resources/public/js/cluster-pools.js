...
$(document).ready(function() {
  ...
  $.get(appUrl(true,"/api/cluster-pool-instances/" + currentClusterName()), function(clusterPoolInstances) {
  ...
  function displayClusterPoolInstances(clusterPoolInstances, problemInstances, problemInstancesMap) {
    ...
    for (var pool in clusterPoolInstances.Detail) {
      if (clusterPoolInstances.Detail.hasOwnProperty(pool)) {
        ...
        pools.push({
          name: pool,
          instances: clusterPoolInstances.Detail[pool]
        });
      }
    }
  ...
  $("#dropdown-context").append('<li><a href="' + appUrl(true,'/web/cluster/' + currentClusterName()) + '">Topology</a></li>');
