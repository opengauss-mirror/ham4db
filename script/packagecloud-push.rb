...
package_files = Dir.glob("/tmp/ham4db-release/**/*.rpm") + Dir.glob("/tmp/ham4db-release/**/*.deb")
package_files.each do |full_path|
  ...
  distro_names.map do |distro_name|
    ...
    result = $client.put_package("ham4db", pkg, distro_id)
...
package_files.each do |full_path|
  ...
  next unless (full_path =~ /\/ham4db[-|_]\d/) or (full_path =~ /\/ham4db-cli[-|_]\d/) or (full_path =~ /\/ham4db-client[-|_]\d/)
  ...
  puts "[#{os}](https://packagecloud.io/#{packagecloud_user}/ham4db/packages/#{distro}/#{File.basename(full_path)}/download)"
end
