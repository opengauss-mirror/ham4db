...
The de-facto configuration list is located in [config.go](https://gitee.com/opengauss/ham4db/blob/master/go/config/config.go).

You are undoubtedly interested in configuring some basic components: the backend database, hosts discoveries. You may choose to use Pseudo-GTID. You may want `ham4db` to notify upon failure, or you may wish to run full blown automated recovery.

Use the following small steps to configure `ham4db`:
...
- [Raft](configuration-raft.md): configure a [ham4db/raft](raft.md) cluster for high availability
...
- [Hints on some settings suitable for larger ham4db environments](configuration-large.md)

### Configuration sample file

For your convenience, this [sample config](configuration-sample.md) is a redacted form of production `ham4db` config at GitHub.
