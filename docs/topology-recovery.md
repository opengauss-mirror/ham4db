...
`ham4db` is able to recover from a set of [failure scenarios](failure-detection.md). Notably, it can recover a failed master or a failed intermediate master.
...
- Topology healing is managed by `ham4db` and is state-based rather than configuration-based. `ham4db` tries to make the best out of a bad situation, taking into consideration the existing topology, versions, server configurations, etc.
...
- Some servers may be lost in the process. `ham4db` must determine which, if any.
...
- Find the best replica to promote.
  ...
  - `ham4db` attempts to promote a replica that will retain the most serving capacity.
...
- DNS based discovery; `ham4db` will need to invoke a hook that modifies DNS entries.
- ZooKeeper/Consul KV/etcd/other key-value based discovery; `ham4db` has built-in support for Consul KV, otherwise an external hook must update KV stores
- Proxy based discovery; `ham4db` will call upon external hook that modifies proxy config, or will update Consul/Zk/etcd as above, which will itself trigger an update to proxy configuration.
- Other solutions...

`ham4db` attempts to be a generic solution hence takes no stance on your service discovery method.
...
-  either the user or `ham4db` choose an existing replica as the designated new master.
- `ham4db` ensures the designated replica takes over its siblings as intermediate master
- `ham4db` turns the master to be `read-only` (possibly also `super-read-only`)
- `ham4db` makes sure your designated server is caught up with replication.
- `ham4db` promotes your designated server as the new master.
- `ham4db` turns promoted server to be writable.
- `ham4db` demotes the old master and places it as a direct replica of the new master
- if possible, `ham4db` sets replication user/password for the demoted master
- in the `graceful-master-takeover-auto` variant (see following), `ham4db` starts replication on demoted master.
...
`ham4db` provides you with specialized hooks to run a graceful takeover:

- `PreGracefulTakeoverProcesses`
- `PostGracefulTakeoverProcesses`

These hooks run _in addition_ to the standard hooks. `ham4db` will run `PreGracefulTakeoverProcesses`, then go through a `DeadMaster` flow, running the normal pre-, post- hooks for `DeadMaster`, and at last follows up with `PostGracefulTakeoverProcesses`.
...
- `graceful-master-takeover`:
  ...
  - the demoted master is placed as replica to the new master, but `ham4db` does not start replication on the server.
- `graceful-master-takeover-auto`:
  - The user _may_ indicate the designated replica to be promoted, which `ham4db` must respect
  - or, the user may omit the identity of designated replica, in which case `ham4db` picks the best replica to promote
  - the demoted master is placed as replica to the new master, and `ham4db` starts replication on the server.

Invoke graceful takeover via:

- Command line; examples:
  - `ham4db-client -c graceful-master-takeover -alias mycluster -d designated.master.to.promote:3306`: indicate designated replica; `ham4db` does not start replication on demoted master
  - `ham4db-client -c graceful-master-takeover-auto -alias mycluster -d designated.master.to.promote:3306`: indicate designated replica; `ham4db` starts replication on demoted master
  - `ham4db-client -c graceful-master-takeover-auto -alias mycluster`: let `ham4db` choose replica to promote; `ham4db` starts replication on demoted master
...
- Web API; examples:
  ...
  - `/api/graceful-master-takeover-auto/:clusterHint`: gracefully promote a new master (planned failover). `ham4db` picks replica to promote. `ham4db` starts replication on demoted master:
...
You may choose to ask `ham4db` to recover a failure by providing a specific instance that is failed.
...
* Command line: `ham4db-client -c recover -i dead.instance.com:3306 --debug`
...
TL;DR force master failover _right now_ regardless of what `ham4db` thinks.

Perhaps `ham4db` doesn't see that the instance is failed, or you have some app-logic that requires the master must change _right now_, or perhaps the type of failure is such that `ham4db` is unsure about. You wish to kick a master failover _right now_. You will run:

* Command line: `ham4db-client -c force-master-failover --alias mycluster`

  or `ham4db-client -c force-master-failover -i instance.in.that.cluster`
...
- `/api/disable-global-recoveries`: global switch to disable `ham4db` from running any recoveries
...
- `/api/recover/:host/:port`: recover specific host, assuming `ham4db` agrees there is failure.
...
- `ham4db-client -c recover -i some.instance:3306`
- `ham4db-client -c graceful-master-takeover -i some.instance.in.somecluster:3306`
- `ham4db-client -c graceful-master-takeover -alias somecluster`
- `ham4db-client -c force-master-takeover -alias somecluster`
- `ham4db-client -c ack-cluster-recoveries -alias somecluster`
- `ham4db-client -c ack-all-recoveries`
- `ham4db-client -c disable-global-recoveries`
- `ham4db-client -c enable-global-recoveries`
- `ham4db-client -c check-global-recoveries`

## Blocking, acknowledgements, anti-flapping

`ham4db` avoid flapping (cascading failures causing continuous outage and elimination of resources) by introducing a block period, where on any given cluster, `orchesrartor` will not kick in automated recovery on an interval smaller than said period, unless cleared to do so by a human.
...
Acknowledging a recovery is possible either via web API/interface (see audit/recovery page) or via command line interface (`ham4db-client -c ack-cluster-recoveries -alias somealias`).

Note that manual recovery (e.g. `ham4db-client -c recover` or `orchstrator-client -c force-master-failover`) ignores the blocking period.
...
```
ham4db-client -c register-candidate -i ${::fqdn} --promotion-rule ${promotion_rule}
```
...
an instance. An instance can be downtimed (via `ham4db-client -c begin-downtime`) and this is noted in the analysis summary. When considering automated recovery, downtimed servers are skipped.
...
Note that manual recovery (e.g. `ham4db-client -c recover`) overrides downtime.

## Recovery hooks

`ham4db` supports hooks -- external scripts invoked through the recovery process. These are arrays of commands invoked via shell, in particular `bash`. See hook configuration details in [recovery configuration](configuration-recovery.md#hooks)
