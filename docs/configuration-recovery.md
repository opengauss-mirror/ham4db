...
- `ApplyMySQLPromotionAfterMasterFailover`: when `true`, `ham4db` will `reset slave all` and `set read_only=0` on promoted master. Default: `true`. When `true`, overrides `MasterFailoverDetachSlaveMasterHost`.
