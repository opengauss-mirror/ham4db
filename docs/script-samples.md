...
This document presents scripting usage and ideas around `ham4db`.
...
```shell
$ ham4db-client -c list-cluster-alias | cut -d"," -f2 | sort
cl1
mycluster
olap
oltp1
...
```shell
$ ham4db-client -c cluster-master -alias mycluster
...
```shell
$ ham4db-client -c list-cluster-instance -alias mycluster
mysql-0246.dc1.domain.net:3306
...
The above indicates what `ham4db` knows about the replication graph. The list includes servers that may be offline/broken.

#### Shell loop over instances

```shell
$ ham4db-client -c list-cluster-instance -alias mycluster | cut -d":" -f 1 | while read h ; do echo "Host is $h" ; done
...
```shell
$ ham4db-client -c list-cluster-instance -alias mycluster | while read i ; do
...
```shell
$ ham4db-client -c cluster-master -alias mycluster | while read i ; do
  ham4db-client -c enable-semi-sync-master -i $i
done
mysql-0246.dc1.domain.net:3306
```
...
```shell
$ master=$(ham4db-client -c cluster-master -alias mycluster)
$ ham4db-client -c list-cluster-instance -alias mycluster | grep -v $master | while read i ; do
  ham4db-client -c disable-semi-sync-master -i $i
done
```
...
```shell
$ ham4db-client -c list-cluster-instance -alias mycluster | grep -v $master | while read i ; do
  ham4db-client -c set-read-only -i $i
done
```

#### We don't really need to loop. We can use ccql

[ccql](https://github.com/github/ccql) is a concurrent, multi server MySQL client. It plays well with scripting in general and with `ham4db` in particular.

```shell
$ ham4db-client -c list-cluster-instance -alias mycluster | grep -v $master | ccql -C ~/.my.cnf -q "set global read_only=1"
```

#### Extract master hostname (no ":3306")

```shell
$ master_host=$(ham4db-client -c cluster-master -alias mycluster | cut -d":" -f1)
$ echo $master_host
...
```shell
$ ham4db-client -c api -path instance/$master_host/3306 | jq .
{
  "Key": {
...
```shell
$ ham4db-client -c api -path instance/$master_host/3306 | jq .Key.Hostname -r
mysql-0246.dc1.domain.net
```
...
```shell
$ ham4db-client -c api -path instance/$master_host/3306 | jq .MasterKey.Hostname -r
...
```shell
$ ham4db-client -c api -path cluster/alias/mycluster | jq .[].Key.Hostname -r
mysql-0246.dc1.domain.net
...
```shell
$ ham4db-client -c api -path cluster/alias/mycluster | jq .[].MasterKey.Hostname -r
...
```shell
$ ham4db-client -c api -path instance/mysql-bb00.dc1.domain.net/3306 | jq .MasterKey.Hostname -r
mysql-00ff.dc1.domain.net
```
...
```shell
$ ham4db-client -c api -path instance/$master_host/3306 | jq '.Replicas | length'
3
```
...
```shell
$ ham4db-client -c api -path cluster/alias/mycluster | jq '.[].Replicas | length'
3
0
2
1
...
```shell
$ ham4db-client -c list-cluster-instance -alias mycluster | ccql -C ~/.my.cnf -q "show slave status" | awk '{print $1}'
mysql-00ff.dc1.domain.net:3306
mysql-bb00.dc1.domain.net:3306
mysql-2222.dc1.domain.net:3306
mysql-ecec.dc2.domain.net:3306
...
```shell
$ ham4db-client -c list-cluster-instance -alias mycluster | ccql -C ~/.my.cnf -q "show slave status" | awk '{print $1}' | ccql -C ~/.my.cnf -q "stop slave; start slave;"
```
...
```shell
$ ham4db-client -c restart-replica-statements -i mysql-bb00.dc1.domain.net -query "change master to auto_position=1" | jq .[] -r
...
```shell
$ ham4db-client -c stop-replica -i mysql-bb00.dc1.domain.net
mysql-bb00.dc1.domain.net:3306
...
```shell
ham4db-client -c restart-replica-statements -i mysql-bb00.dc1.domain.net -query "change master to auto_position=1" | jq .[] -r | mysql -h mysql-bb00.dc1.domain.net
```
...
```shell
$ ham4db-client -c api -path instance/mysql-bb00.dc1.domain.net/3306 | jq '.DataCenter'
dc1
```
...
```shell
$ ham4db-client -c api -path cluster/mycluster | jq '.[].DataCenter' -r | sort | uniq -c
  4 dc1
  3 dc2
```
...
```shell
$ ham4db-client -c api -path cluster/mycluster |
    ...
    while read h dc m ; do
      ham4db-client -c api -path "instance/$m" | jq '.DataCenter' -r |
