...
When operating in HTTP mode (API or Web), access to `ham4db` may be restricted via either:

*  _basic authentication_

   Add the following to `ham4db`'s configuration file:
   ...
   `Ham4DB`'s configuration file contains credentials to your MySQL servers as well as _basic authentication_
...
*  _Headers authentication_
   ...
   Authenticates via headers forwarded by reverse proxy (e.g. Apache2 relaying requests to ham4db).
   Requires:
...
Or, regardless, you may turn the entire `ham4db` process to be read only via:
