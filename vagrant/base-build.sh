...
if [[ -e /etc/redhat-release ]]; then
  ...
  # Build ham4db and ham4db agent
  mkdir -p /home/vagrant/go/{bin,pkg,src} /tmp/ham4db-release
  mkdir -p /home/vagrant/go/src/gitee.com/opengauss/ham4db
  mount --bind /ham4db /home/vagrant/go/src/gitee.com/opengauss/ham4db
  ...
  /usr/bin/go get ./...
  ${GOPATH}/src/gitee.com/opengauss/ham4db/build.sh
  chown -R vagrant.vagrant /home/vagrant /tmp/ham4db-release
  ...
  if [[ -e "/ham4db/vagrant/${HOSTNAME}-my.cnf" ]]; then
    rm -f /etc/my.cnf
    cp /ham4db/vagrant/${HOSTNAME}-my.cnf /etc/my.cnf
  fi
...
GRANT ALL PRIVILEGES ON ham4db.* TO 'orc_client_user'@'%' IDENTIFIED BY 'orc_client_password';
GRANT SUPER, PROCESS, REPLICATION SLAVE, RELOAD ON *.* TO 'orc_client_user'@'%';
GRANT ALL PRIVILEGES ON ham4db.* TO 'orc_client_user'@'localhost' IDENTIFIED BY 'orc_client_password';
GRANT SUPER, PROCESS, REPLICATION SLAVE, RELOAD ON *.* TO 'orc_client_user'@'localhost';
GRANT ALL PRIVILEGES ON ham4db.* TO 'orc_server_user'@'localhost' IDENTIFIED BY 'orc_server_password';
EOF
...
if [[ $HOSTNAME == 'admin' ]]; then
  bash /ham4db/vagrant/admin-build.sh
  if [[ -e /ham4db/vagrant/admin-post-install.sh ]]; then
    bash /ham4db/vagrant/admin-post-install.sh
  fi
else
  bash /ham4db/vagrant/$HOSTNAME-build.sh

  if [[ -e /ham4db/vagrant/db-post-install.sh ]]; then
    bash /ham4db/vagrant/db-post-install.sh
  fi

  if [[ -e /ham4db/vagrant/$HOSTNAME-post-install.sh ]]; then
    bash /ham4db/vagrant/$HOSTNAME-post-install.sh
  fi
fi
