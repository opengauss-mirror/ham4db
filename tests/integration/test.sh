...
# See https://gitee.com/opengauss/ham4db/tree/doc/local-tests.md
#
...
test_logfile=/tmp/ham4db-test.log
test_outfile=/tmp/ham4db-test.out
test_diff_file=/tmp/ham4db-test.diff
test_query_file=/tmp/ham4db-test.sql
test_config_file=/tmp/ham4db.conf.json
ham4db_binary=/tmp/ham4db-test
exec_command_file=/tmp/ham4db-test.bash
test_mysql_defaults_file=/tmp/ham4db-test-my.cnf
db_type=""
sqlite_file="/tmp/ham4db.db"
...
function run_queries() {
  ...
  if [ "$db_type" == "sqlite" ] ; then
    cat $queries_file |
      sed -e "s/last_checked_timestamp - interval 1 minute/datetime('last_checked_timestamp', '-1 minute')/g" |
...
build_binary() {
  echo "Building"
  GO111MODULE=auto go build -o $ham4db_binary go/app/main.go
}
...
deploy_internal_db() {
  ...
  echo "- meta-deploy_internal_db result: $?"
}

generate_config_file() {
  cp ${tests_path}/ham4db.conf.json ${test_config_file}
