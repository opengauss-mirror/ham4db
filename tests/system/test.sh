...
setup_teardown_logfile=/tmp/ham4db-setup-teardown.log
...
test_restore_outfile=/tmp/ham4db-test-restore.out
test_restore_diff_file=/tmp/ham4db-test-restore.diff
...
check_environment() {
  ...
  echo "checking ham4db-client"
  if ! which ham4db-client ; then
    echo "+ not found in PATH"
    if [ -f resources/bin/ham4db-client ] ; then
      echo "found in resources/bin. Updating PATH"
      export PATH="$PATH:$(pwd)/resources/bin"
    else
      echo "ham4db-client not found"
      exit 1
    fi
  fi
...
test_step() {
  ...
  if [ -f $test_path/config.json ] ; then
    ...
    ham4db-client -c api -path "reload-configuration?config=$real_config_path" | jq -r '.Code'
  fi
